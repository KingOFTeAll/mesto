// "import massiveElementsPlaces from "./Massivecards";"
...
const handleCloseByClick = (evt) => { //оверлейчик
  if ((evt.target == evt.currentTarget) || (evt.target.classList.contains("popup__close-img"))) {
...
const handleSubmitProfileForm = function (event) {//Функция присвоения значений из попапа в профиль
...
const createElementPlace = function (card) { //make карточки
  const cardsItem = elementTemplate.querySelector(".elements__item").cloneNode(true);
  ...
  cardsItem.querySelector(".elements__trash").addEventListener("click", (event) => {
