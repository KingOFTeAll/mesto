...
let inputProfileName = popupElement.querySelector('.form__input_type-name');
let inputProfileStatus = popupElement.querySelector('.form__input_type-status'); 
...
const renameingProfileandStatus = function(event){
   ...
   closePopup();
   event.preventDefault();// Как я понял при сабмите дефолтное поведение-перезагрузка страницы,если убрать это,то value остается
}
